...
- 模型类型以紧凑图标标识，并始终提供 Tooltip 文字；获取结果与已保存模型都显示同一类型标识。自动识别只作为初始值，用户可在模型行内改为文本、多模态、语音、嵌入、重排或图像类型。
- MCP 添加服务器弹窗提供「表单填写 / JSON 输入」两个互斥入口。JSON 入口一次只解析一个服务器；HTTP 表单在 URL 下方提供 JSON 请求头，错误必须就近显示且不得静默丢弃配置。
- Max tokens 使用固定预设（200K、220K、273K、300K、1M）或自定义模式；仅自定义模式展示数值输入框，新建供应商默认 220K。
