...
**最后审阅 / Last reviewed:** 2026-08-07（v32）
...
- Skills 的唯一入口是 `Settings > Skills`；不得恢复 `{ page: "skills" }`、独立 title/nav 分支或页面 wrapper，也不得在 `UserMenu` / 任务侧栏底栏重复暴露 Skills 入口。领域 UI 必须由 `SkillsSettingsFeature` 承载。
