...
| 文件 | 用途 |
|------|------|
| `default.json` | 只绑定本地 `main` window 的最小 ACL：event listen/unlisten、经校验的 HTTP(S) 外链 open、dialog、clipboard、`main-commands` 与五个 `terminal-runtime` commands；不含通用 shell execute/spawn/stdin/kill、FS、HTTP 或 Notification 权限 |

### `src-tauri/permissions/`

| 文件 | 用途 |
|------|------|
| `main.toml` | 普通 custom commands 的精确 permission 集合 |
| `terminal.toml` | 仅包含 `terminal_spawn/write/resize/kill/get_state` 的窄 Terminal runtime permission |
| `autogenerated/` | Tauri 为单个 custom command 生成的权限描述；与 invoke handler/AppManifest 的集合由静态测试锁定 |
