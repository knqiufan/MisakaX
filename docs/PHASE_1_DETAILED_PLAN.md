...
- [x] **F-1** 完整冒烟测试：启动应用 → 导航所有页面 → 修改设置 → 切换主题 → 切换语言
- [x] **F-2** 持久化测试：关闭应用 → 重新打开 → 之前的设置仍然保留
- [x] **F-3** 多次主题切换测试：亮→暗→系统→暗→亮，确认无闪烁/残留
- [x] **F-4** Provider 安全测试：用 sqlite3 查看数据库确认 API Key 已加密存储（非明文）
- [x] **F-5** 对照第 13 节验证清单，逐项确认全部通过
