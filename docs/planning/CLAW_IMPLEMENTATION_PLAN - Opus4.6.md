...
> **M1: 核心可用** — Phase 2 完成后，MisakaX 已经是一个可日常使用的 AI 对话客户端。
...
引入 Python Sidecar，实现 LangGraph Agent 编排 + PowerMem 智能记忆。这是 MisakaX 从"对话客户端"进化为"Agent 平台"的关键阶段。
...
> **M3: Agent 平台** — MisakaX 具备独立的 Agent 编排和记忆能力，不依赖任何外部 Agent SDK。
