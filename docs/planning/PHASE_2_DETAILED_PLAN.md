...
- [x] **F-1** 工作目录冒烟测试：新建会话 → 选择工作目录 → WorkspaceBar 显示正确 → 切换目录
- [x] **F-2** 完整对话冒烟测试：创建会话 → 发送消息 → 流式回复 → 切换会话 → 重启验证持久化
- [x] **F-3** 三 Provider 测试：分别使用 OpenAI / Anthropic / Gemini 完整对话（代码路径已具备；**手工**确认见 `docs/guides/phase2-e2e-verification-matrix.md`）
- [x] **F-4** 思维链测试：使用 Claude 模型验证 thinking blocks 流式显示（**手工**确认同上矩阵）
- [x] **F-5** 控制流测试：停止生成 → 重新生成 → 多轮对话连续性
- [x] **F-6** 多模态测试：粘贴截图 → 发送给视觉模型 → 验证理解正确（**手工**确认同上矩阵）
- [x] **F-7** 错误处理测试：使用无效 API Key 发送消息 → 友好错误提示
- [x] **F-8** 性能验证：长回复（1000+ token）流式输出不卡顿（自动化长 delta 测试 + **手工**主观确认）
- [x] **F-9** 对照第 18 节验证清单，逐项确认全部通过（记录见 `docs/guides/phase2-e2e-verification-matrix.md`）
